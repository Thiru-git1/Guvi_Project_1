...
# --- Streamlit App ---
st.title("IMDb 2024 Movie Analysis Visualizations Dashboard")
...
query_genre = """SELECT Genre, COUNT(*) AS Movie_Count
FROM IMDB_Movies_2024
GROUP BY Genre
...
query_avg_duration = """SELECT Genre, AVG(Duration) AS Duration
FROM IMDB_Movies_2024
GROUP BY Genre
...
fig_duration = px.bar(avg_dur_df,x="Duration",y="Genre",title="Average Movie Duration by Genre",
    color="Duration",orientation='h',
    labels={"Duration": "Avg Duration (mins)"})
...
query_votes = """SELECT Genre, AVG(Voting_counts) AS Avg_Votes
FROM IMDB_Movies_2024
GROUP BY Genre
ORDER BY Avg_Votes DESC;
"""
votes_df = pd.read_sql(query_votes, engine)
fig_votes = px.bar(votes_df,x="Avg_Votes",y="Genre",orientation="h",color="Avg_Votes",title="🗳️ Average Voting Counts by Genre",
    labels={"Avg_Votes": "Average Votes"},height=600,width=1000)
st.plotly_chart(fig_votes)

# 5. Rating Distribution: Analyze the distribution of ratings across all movies.
...
# Get top-rated movie per genre using pandas
top_movies_by_genre = df.loc[df.groupby('Genre')['Ratings'].idxmax()]
...
# Run the SQL query
query_avg_rating = """ SELECT Genre, AVG(Ratings) AS Avg_Rating 
FROM IMDB_Movies_2024 GROUP BY Genre
...
query_popular_genres = """SELECT Genre, SUM(Voting_counts) AS Total_Votes 
FROM IMDB_Movies_2024 GROUP BY Genre
...
"""
# Fetch the data
ratings_by_genre_df = pd.read_sql(query_ratings_by_genre, engine)
...
query_correlation = """ SELECT Genre,Ratings, Voting_counts, Title
FROM IMDB_Movies_2024
WHERE Ratings IS NOT NULL AND Voting_counts IS NOT NULL
ORDER BY Voting_counts DESC;
"""
# Fetch the data
correlation_df = pd.read_sql(query_correlation, engine)
# Create scatter plot using Plotly
fig_correlation = px.scatter(correlation_df,x="Ratings",y="Voting_counts",hover_name="Title",
    title="Correlation Between Ratings and Voting Counts",
    labels={"Ratings": "IMDb Rating", "Voting_counts": "Number of Votes"},
    color="Genre",  # Color based on ratings
